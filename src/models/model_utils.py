...
def get_score_fn(model, train=False, sampling=False):
    ...
    with torch.cuda.amp.autocast(dtype=torch.bfloat16):
        ...
        def score_fn(x, labels, sigma):
            ...
            output = model_fn(x, labels, sigma)

            # Check if model returned a tuple (when attention scoring is enabled)
            if isinstance(output, tuple):
                score = output[0]  # First element is the actual score
                attention_scores = output[1]  # Second element is attention scores
            else:
                score = output
            ...
            if sampling:
                ...
                if attention_scores is not None:
                    return score.exp(), attention_scores
                else:
                    return score.exp()
